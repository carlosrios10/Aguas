...
carga el modelo calibrado (lgbm_model_cal.pkl) y features, obtiene scores P(fraud) y guarda data/predictions/scores_<CUTOFF>.csv.
"""
import argparse
import logging
import os
...
def setup_logging(log_dir=None, log_level=logging.INFO):
    """Configura logging: consola + archivo (si log_dir). Devuelve logger."""
    ...
    try:
        ...
        if df is None:
            logger.error(
                ...
                "Revise que existan datos en interim (consumo) y que inference.cutoff tenga formato YYYY-MM-DD."
            )
            return 1
        ...
        logger.info("Paso 3/5: Cargando modelo calibrado y lista de features...")
        ...
        if missing:
            logger.error(
                ...
                "Asegúrese de haber entrenado el modelo (run_train.py; genera lgbm_model_cal.pkl) y de que features.pkl coincida con el dataset.",
                missing,
            )
            return 1
        ...
        logger.exception(
            "La inferencia falló. Compruebe config (inference.cutoff, paths) y que existan datos en interim y models/lgbm_model_cal.pkl."
        )
        return 1
