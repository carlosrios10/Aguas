...
4. Busca la línea que dice **`cutoff`** dentro de la sección **`inference`**. Verás algo como:
   ...
   inference:
     cutoff: "2025-06-01"
     cant_periodos: 12
     # ... más opciones (no es necesario cambiarlas)
   ```
...
- **Importante:** Tienes que estar en la **carpeta raíz del proyecto** (la que contiene las carpetas `config`, `scripts`, `data`, etc.).  
  - Si abriste la terminal desde otra carpeta, escribe algo como (ajusta la ruta a tu PC):
    ```text
    cd C:\ruta\donde\esta\el\proyecto\ar-epm_poc
    ```
  - y pulsa Enter.
...
- **Nombre del archivo:** `scores_AAAA-MM-DD_HHMMSS.csv`  
  La primera parte es la fecha que pusiste en `cutoff`; la segunda es la hora en que ejecutaste el proceso (para no sobrescribir ejecuciones anteriores).  
  Ejemplo: si usaste `"2025-01-01"` y ejecutaste el 6 de febrero de 2025 a las 14:30:22, el archivo se llamará **`scores_2025-01-01_20250206_143022.csv`** (la parte final es fecha y hora de esa ejecución).
...
| 5 | Buscar el archivo en `data/predictions/` (nombre tipo `scores_AAAA-MM-DD_HHMMSS.csv`). |
| 6 | Abrirlo en Excel, ordenar por **score** de mayor a menor y usar esa lista para priorizar inspecciones. |

---

## Ejecución alternativa: desde el notebook (Jupyter Lab)

Si prefieres ejecutar la inferencia desde un **notebook** en lugar del comando en la terminal, puedes usar **Jupyter Lab**. El resultado es el mismo: el archivo de puntajes se guarda en `data/predictions/` con el mismo formato de nombre.

**Antes de empezar:** haz el **Paso 1** (cambiar la fecha `cutoff` en `config/config.yaml`). Luego sigue estos pasos.

### 1. Abrir la terminal en la carpeta raíz del proyecto

Igual que en el Paso 2: la terminal debe estar en la carpeta que contiene `config`, `scripts`, `poc`, `data`, etc. Si no, usa `cd` para entrar (por ejemplo: `cd C:\ruta\al\proyecto\ar-epm_poc`).
...
- Se abrirá una ventana o pestaña en tu **navegador** (Chrome, Edge, etc.) con la interfaz de Jupyter Lab.
- **No cierres la terminal** mientras trabajes con el notebook; si la cierras, Jupyter se detendrá.

### 4. Abrir el notebook de inferencia

En Jupyter Lab (en el navegador):

1. En el panel de la izquierda, entra en la carpeta **`poc`**.
2. Haz doble clic en el archivo **`inference.ipynb`** (o clic derecho → Open With → Notebook).

Se abrirá el notebook con varias celdas (bloques de código y texto).

### 5. Ejecutar todas las celdas

Para ejecutar todo el proceso de inferencia:

- En el menú superior: **Run** → **Run All Cells** (o “Ejecutar todas las celdas”),  
  **o**
- Ir celda por celda y pulsar el botón de **reproducir** (▶) o la combinación **Shift + Enter**.

El proceso puede tardar unos minutos. Verás mensajes de avance en las celdas. Al final, en la celda que guarda el CSV aparecerá un mensaje tipo **“[OK] Guardado: …”** con la ruta del archivo.

### 6. Dónde está el archivo de resultados

Igual que en la ejecución por script: en la carpeta **`data/predictions/`**, con nombre del tipo **`scores_AAAA-MM-DD_HHMMSS.csv`**. Puedes abrirlo en Excel y usarlo como se explica en el Paso 6 de la guía principal.

**Resumen de esta alternativa:** (1) Editar `cutoff` en config. (2) Terminal en la raíz del proyecto. (3) Activar entorno si aplica. (4) Ejecutar `jupyter lab`. (5) En el navegador, abrir `poc/inference.ipynb` y ejecutar todas las celdas. (6) Buscar el CSV en `data/predictions/`.
...
- **Hay varios archivos scores_... en la carpeta**  
  Cada ejecución genera un archivo con fecha y hora en el nombre (por ejemplo `scores_2025-01-01_20250206_143022.csv`). El más reciente es el de la última vez que ejecutaste el proceso.

- **“jupyter no se reconoce” o no arranca Jupyter Lab**  
  Asegúrate de haber activado el entorno virtual (paso 2 de la sección del notebook) y de estar en la raíz del proyecto. Si sigue sin funcionar, el equipo técnico puede instalar Jupyter en tu entorno (`pip install jupyterlab`) o indicarte cómo abrir el notebook de otra forma.

---

**Documento:** Guía de inferencia paso a paso (personas no técnicas)  
**Proyecto:** AR-EPM POC (ar-epm_poc)
