...
Para el flujo mensual completo (ETL + inferencia), requisitos de datos y solución de problemas, consulte [manual_usuario.md](manual_usuario.md).
...
- **Nombre del archivo:** `scores_AAAA-MM-DD_AAAAMMDD_HHMMSS.csv`  
  La primera parte es la fecha que pusiste en `cutoff`; las dos siguientes son la fecha y hora en que ejecutaste el proceso (para no sobrescribir ejecuciones anteriores).  
  Ejemplo: si usaste `"2025-01-01"` y ejecutaste el 6 de febrero de 2025 a las 14:30:22, el archivo se llamará **`scores_2025-01-01_20250206_143022.csv`**.
...
| 5 | Buscar el archivo en `data/predictions/` (nombre tipo `scores_AAAA-MM-DD_AAAAMMDD_HHMMSS.csv`). |
...
Igual que en la ejecución por script: en la carpeta **`data/predictions/`**, con nombre del tipo **`scores_AAAA-MM-DD_AAAAMMDD_HHMMSS.csv`**. Puedes abrirlo en Excel y usarlo como se explica en el Paso 6 de la guía principal.
...
- **Hay varios archivos scores_... en la carpeta**  
  Cada ejecución genera un archivo con fecha y hora en el nombre (por ejemplo `scores_2025-01-01_20250206_143022.csv`). El más reciente (por fecha de modificación) es el de la última vez que ejecutaste el proceso.
